...
const title = 'LabQueue';

function createOne(obj, session) {
    return LabQueue.insertMany(obj, { session });
}

function getAll() {
    return LabQueue.find().exec();
}

function deleteOne(id, session) {
    return LabQueue.findByIdAndDelete(id).session(session).exec();
}

function getByCondition(condition) {
    return LabQueue.find(condition).exec();
}


function getById(id) {
    return LabQueue.findById(id).exec();
}
